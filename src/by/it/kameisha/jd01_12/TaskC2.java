...
public class TaskC2 {
    ...
    private static Set<Number> getCross(Set<? extends Number>... sets) {
        Set<? extends Number>[] numbers = sets;
        Comparator<Number> comparator = Comparator.comparing(Number::doubleValue);
        Set<Number> numbersCross = new HashSet<>();
        if (numbers.length == 1) {
            return (Set<Number>) numbers[0];
        } else {
